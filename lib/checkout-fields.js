...
const LEGACY_FIELD_SOCIAL = "social_profile";
...
export function checkoutFieldsFromSession(session) {
  ...
  for (const field of session?.custom_fields || []) {
    ...
    if (field?.key === LEGACY_FIELD_SOCIAL || field?.key === "social_url") {
      legacySocialRaw = value.slice(0, 220);
    }
  }
  ...
  const socialUrl =
    parseSocialUrl(legacySocialRaw) ||
